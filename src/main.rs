...
pub fn input_sets<S: ::std::hash::BuildHasher>(
    ...
    for m in &mut set_matches.iter() {
        ...
        **/
        let ths: f64 = Stats::total_scoring_by_club_to_date(
            &league_stats.get(&m.home).unwrap(),
            Scoring::Home,
        )
        .into();
        let tas: f64 = Stats::total_scoring_by_club_to_date(
        ...
        **/
        let hsh: f64 = Stats::highest_alltime_scores_by_club(&league_stats.get(&m.home).unwrap())
            .0
            .into();
        let hsa: f64 = Stats::highest_alltime_scores_by_club(&league_stats.get(&m.away).unwrap())
            .1
            .into();
